...
int GetMessageBuffer(void){
  ...
  // if(msg_no!=EndOfFreeList){free_msg_buffer=message_buffer[msg_no][0];}
  free_msg_buffer++;
  return msg_no;
}
...
{
    struct Queue* queue = (struct Queue*) kalloc();
    queue->capacity = capacity;
    queue->front = queue->size = 0;
    queue->rear = capacity - 1;  // This is important, see the enqueue
    queue->array = (int*) kalloc();
    return queue;
}
...
void intitialize_queues(){
  for (int i = 1; i <= NPROC; i++) {
    /* code */
    message_queues[i]=m_queue(100);
    wait_queues[i]=m_queue(100);
  }
}
...
print_count()
{
  const char* sys_call_names[]={"sys_add ", "sys_chdir ", "sys_close ", "sys_dup ", "sys_exec ", "sys_exit ", "sys_fork ", "sys_fstat ", "sys_getpid ", "sys_kill ", "sys_link ",
  "sys_mkdir ", "sys_mknod ", "sys_open ", "sys_pipe ", "sys_print_count ", "sys_ps ", "sys_read ","sys_recv", "sys_sbrk ","sys_send","sys_send_multi", "sys_sleep ", "sys_toggle ",
...
send()
{
  ...
  // message_buffer[msg_no]=mess;
  // cprintf("Messsage in buffer is %s\n",message_buffer[msg_no]);
